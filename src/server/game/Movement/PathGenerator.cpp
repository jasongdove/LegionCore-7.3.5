...
void PathGenerator::InitPath()
{
    ...
    if (DisableMgr::IsPathfindingEnabled(mapId))
    {
        ...
        if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
        {
            ...
            TC_LOG_DEBUG("maps.mmaps", "PathGenerator 0 displayId %i _transport %i _navMeshQuery %i _navMesh %i", _go ? _go->GetDisplayId() : 0, _transport ? _transport->GetDisplayId() : 0, bool(_navMeshQuery), bool(_navMesh));
        }
        ...
        if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
            TC_LOG_DEBUG("maps.mmaps", "PathGenerator 1 displayId %i _transport %i _navMeshQuery %i _navMesh %i", _go ? _go->GetDisplayId() : 0, _transport ? _transport->GetDisplayId() : 0, bool(_navMeshQuery), bool(_navMesh));
    }
}
...
bool PathGenerator::CalculatePath(float destX, float destY, float destZ, bool forceDest, bool straightLine, bool stopOnlyOnEndPos)
{
    ...
    // if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
        // TC_LOG_DEBUG("maps.mmaps", "CalculatePath ::0 startPos %f %f %f endPos %f %f %f", x, y, z, destX, destY, destZ);
    ...
    // if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
        // TC_LOG_DEBUG("maps.mmaps", "CalculatePath :: 1 startPos %f %f %f endPos %f %f %f", x, y, z, destX, destY, destZ);
    ...
    // if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
        // TC_LOG_DEBUG("maps.mmaps", "++ PathGenerator::CalculatePath() for %s _navMesh %u _navMeshQuery %u start %u dest %u",
...
bool PathGenerator::CalculateShortcutPath(float destX, float destY, float destZ)
{
    ...
    if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
        TC_LOG_DEBUG("maps.mmaps", "CalculatePath :: startPos %f %f %f endPos %f %f %f", x, y, z, destX, destY, destZ);
    ...
    // if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
        // TC_LOG_DEBUG("maps.mmaps", "++ PathGenerator::CalculateShortcutPath() for %s _navMesh %u _navMeshQuery %u start %u dest %u",
            // _sourceUnit->GetGUID().ToString().c_str(), bool(_navMesh), bool(_navMeshQuery), HaveTile(start), HaveTile(dest));
...
void PathGenerator::BuildPolyPath(G3D::Vector3 const& startPos, G3D::Vector3 const& endPos)
{
    ...
    if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
        TC_LOG_DEBUG("maps.mmaps", "++ BuildPolyPath :: startPos %f %f %f endPos %f %f %f", startPos.y, startPos.z, startPos.x, endPos.y, endPos.z, endPos.x);
    ...
    {
        if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
            TC_LOG_DEBUG("maps.mmaps", "++ BuildPolyPath :: (startPoly == %u || endPoly == %u)", startPoly, endPoly);
    ...
    if (startPoly == endPoly && !_stopOnlyOnEndPos)
    {
        ...
        if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
            TC_LOG_DEBUG("maps.mmaps", "++ BuildPolyPath :: path type %d\n", _type);
        return;
    }
    ...
    if (_polyLength)
    {
        for (; pathStartIndex < _polyLength; ++pathStartIndex)
        {
            ...
            {
                TC_LOG_ERROR("maps.mmaps", "Invalid poly ref in BuildPolyPath. _polyLength: %u, pathStartIndex: %u," " startPos: %s, endPos: %s, mapid: %u", _polyLength, pathStartIndex, startPos.toString().c_str(), endPos.toString().c_str(), _sourceUnit->GetMapId());
                break;
            }
    ...
    else if (startPolyFound && !endPolyFound)
    {
        ...
        if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
            TC_LOG_DEBUG("maps.mmaps", "++  m_polyLength=%u prefixPolyLength=%u suffixPolyLength=%u \n", _polyLength, prefixPolyLength, suffixPolyLength);
...
void PathGenerator::BuildPointPath(const float *startPoint, const float *endPoint)
{
    ...
    if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
        TC_LOG_DEBUG("maps.mmaps", "++ PathGenerator::BuildPointPath path type %d size %d poly-size %d _forceDestination %u", _type, pointCount, _polyLength, _forceDestination);
}
...
void PathGenerator::NormalizePath()
{
    ...
    for (uint32 i = 0, next = 1; next < _pathPoints.size(); ++i, ++next)
    {
        ...
        TC_LOG_DEBUG("maps.mmaps", "PathGenerator::NormalizePath next %u _pathPoints %u vector %f %f %f dist %f 2ddist %f", next, i, _pathPoints[i].x, _pathPoints[i].y, _pathPoints[i].z, dtVdist(lastPoint, nextPoint), dtVdist2D(lastPoint, nextPoint));
    }

    TC_LOG_DEBUG("maps.mmaps", "PathGenerator::NormalizePath _pathPoints %u", _pathPoints.size());
    ...
    for (uint32 i = 0; next < _pathPoints.size(); ++i, ++next)
    {
        ...
        while (!result)
        {
            ...
            TC_LOG_DEBUG("maps.mmaps", "NormalizePath 0 %f %f %f moveCur %f %f %f", lastPoint[0], lastPoint[1], lastPoint[2], moveCur[0], moveCur[1], moveCur[2]);
            ...
            if (dtVdist(lastPoint, moveTgt) <= 2.0f) // min distance 2m for divide patch
            {
                ...
                dtVcopy(moveCur, nextPoint);
                TC_LOG_DEBUG("maps.mmaps", "NormalizePath 0 push_back %u vector %f %f %f dist %f 2ddist %f", _newPathPoints.size(), moveTgt[0], moveTgt[1], moveTgt[2], dtVdist(lastPoint, moveCur), dtVdist2D(lastPoint, moveCur));
                break;
            }
            ...
            if (result)
            {
                ...
                TC_LOG_DEBUG("maps.mmaps", "NormalizePath 1 push_back %u vector %f %f %f dist %f 2ddist %f", _newPathPoints.size(), moveTgt[0], moveTgt[1], moveTgt[2], dtVdist(lastPoint, moveCur), dtVdist2D(lastPoint, moveCur));
            ...
            TC_LOG_DEBUG("maps.mmaps", "NormalizePath 1 %f %f %f moveCur %f %f %f", lastPoint[0], lastPoint[1], lastPoint[2], moveCur[0], moveCur[1], moveCur[2]);
        }
    }
    ...
    for (uint32 i = 0; i < _pathPoints.size(); ++i)
        TC_LOG_DEBUG("maps.mmaps", "NormalizePath _pathPoints %u vector %f %f %f", i, _pathPoints[i].x, _pathPoints[i].y, _pathPoints[i].z);

    TC_LOG_DEBUG("maps.mmaps", "PathGenerator::NormalizePath end _pathPoints %u counter %u", _pathPoints.size(), counter);
}
...
bool PathGenerator::HaveTile(const G3D::Vector3& p) const
{
    ...
    if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
        TC_LOG_ERROR("maps.mmaps", "++ HaveTile tx %i ty %i p %f %f %f",tx, ty, p.y, p.z, p.x);
...
void PathGenerator::ReducePathLenghtByDist(float dist)
{
    ...
    {
        TC_LOG_ERROR("maps.mmaps", "PathGenerator::ReducePathLenghtByDist called before path was built");
        return;
    }
