...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        {
            { "mmap", SEC_GAMEMASTER, true, nullptr, "", mmapCommandTable },
        };
        return commandTable;
    }
    ...
    {
        Player* player = handler->GetSession()->GetPlayer();
        uint32 terrainMapId = player->GetMapId(); // PhasingHandler::GetTerrainMapId(player->GetPhaseShift(), player->GetMapId(), player->GetMap()->GetTerrain(), player->GetPositionX(), player->GetPositionY());
        dtNavMesh const* navmesh = MMAP::MMapFactory::createOrGetMMapManager()->GetNavMesh(terrainMapId);
        dtNavMeshQuery const* navmeshquery = MMAP::MMapFactory::createOrGetMMapManager()->GetNavMeshQuery(terrainMapId, player->GetInstanceId());
        if (!navmesh || !navmeshquery)
        {
