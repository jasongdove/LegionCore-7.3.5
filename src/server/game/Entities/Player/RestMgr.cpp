...
void RestMgr::SetRestBonus(RestTypes restType, float restBonus)
{
    ...
    switch (restType)
    {
        case REST_TYPE_XP:
            ...
            next_level_xp_field = PLAYER_FIELD_NEXT_LEVEL_XP;
            affectedByRaF = true;
            break;
    ...
    if (affectedByRaF && _player->GetsAFriendBonus(true) && (_player->GetSession()->IsARecruiter() || _player->GetSession()->GetRecruiterId() != 0))
...
void RestMgr::RemoveRestFlag(RestFlag restFlag)
{
    ...
    {
        _restTime = 0;
        _player->RemoveFlag(PLAYER_FIELD_PLAYER_FLAGS, PLAYER_FLAGS_RESTING);
    }
}
...
uint32 RestMgr::GetRestBonusFor(RestTypes restType, uint32 xp)
{
    ...
    TC_LOG_DEBUG(LOG_FILTER_PLAYER, "RestMgr::GetRestBonus: Player '%s' (%s) gain %u xp (+%u Rested Bonus). Rested points=%f", _player->GetGUID().ToString().c_str(), _player->GetName(), xp + rested_bonus, rested_bonus, GetRestBonus(restType));
    return rested_bonus;
}
...
float RestMgr::CalcExtraPerSec(RestTypes restType, float bubble) const
{
    ...
    {
        case REST_TYPE_HONOR:
            return static_cast<float>(_player->GetUInt32Value(PLAYER_FIELD_HONOR_NEXT_LEVEL)) / 72000.0f * bubble;
        case REST_TYPE_XP:
            return static_cast<float>(_player->GetUInt32Value(PLAYER_FIELD_NEXT_LEVEL_XP)) / 72000.0f * bubble;
        default:
            return 0.0f;
    }
}
